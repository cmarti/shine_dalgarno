...
if __name__ == '__main__':
    ...
    marginal_sites = pd.read_csv('results/vcregression.map_site_marginal_epistasis.csv', index_col=0)
    marginal_pw = pd.read_csv('results/vcregression.map_pairwise_marginal_epistasis.csv', index_col=0)
    ...
    sns.histplot(x=x, y=y, cmap='viridis', ax=axes, bins=(bins, bins), 
                 cbar_ax=cbar_axes1,
                 cbar=True, cbar_kws={'label': '# sequences ($10^{3}$)', 'shrink': 0.95})
    ...
    x, xerr = test['y_pred'], 2 * np.sqrt(test['y_var'])
    y, yerr = test['y'], 2 * test['std']
    r2 = pearsonr(y, x)[0] ** 2
    predictive_std = np.sqrt(test['y_var'] + test['y_var_pred'])
    lower = test['y_pred'] - 2 * predictive_std
    upper = test['y_pred'] + 2 * predictive_std
    
    calibration = np.logical_and(y < upper, y > lower).mean()
    ...
    axes.text(x1, y3, 'E', fontsize=10, weight='bold', transform=fig.transFigure)
    
    print('Plotting marginal variance components between pairs of sites in the MAP')
    ...
    sns.heatmap(df, ax=axes, cmap="Greys",
                cbar_kws={"label": label, 'shrink':0.8},
                vmin=0, vmax=20,
                cbar_ax=cbar_axes3)
    sns.despine(ax=axes, top=False, right=False)
    ...
    axes.text(x2, y3, 'F', fontsize=10, weight='bold', transform=fig.transFigure)
    axes.set_xticklabels(axes.get_xticklabels(), rotation=90)


    print('Adjusting panel positions')
    fig.subplots_adjust(bottom=0.1, left=0.125, hspace=0.5, wspace=1., top=0.95, right=0.975)
