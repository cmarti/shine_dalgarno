...
if __name__ == "__main__":
    ...
    print("Loading data")
    gene_data = pd.read_csv("processed/e_coli.gene_5utr.csv", index_col=0)
    seqs, counts = np.unique(load_sequence_data("e_coli"), return_counts=True)
    inferred = pd.read_csv("results/e_coli.seqdeft.map.csv", index_col=0)
    logL_df = pd.read_csv("results/e_coli.seqdeft.cv_results.csv", index_col=0)

    print("Preparing data for plotting")
    counts = pd.DataFrame({"counts": counts}, index=seqs)
    inferred = inferred.join(counts).fillna(0)
    ...
    axes.text(
        0.05, 0.95, "A", fontsize=10, weight="bold", transform=fig.transFigure
    )
    ...
    frame.set_alpha(0.5)
    axes.text(
        0.05, 0.65, "B", fontsize=10, weight="bold", transform=fig.transFigure
    )
    ...
    subplots[0].text(
        0.565, 0.65, "C", fontsize=10, weight="bold", transform=fig.transFigure
    )

    fig.subplots_adjust(
        bottom=0.175, left=0.2, hspace=0.15, wspace=0.25, top=0.925, right=0.95
    )
    fig.savefig("figures/Figure_2.jpg", dpi=600)
    fig.savefig("figures/Figure_2.svg", dpi=600)
