...
if __name__ == "__main__":
    pred = pd.read_csv("data/seqdeft_path_contrasts.csv", index_col=0)
    print(pred)

    idx = pd.read_csv("data/peaks_contrasts.csv", index_col=0).index

    peaks_contrasts = pd.read_csv(
        "data/seqdeft_peaks_contrasts.csv", index_col=0
    ).loc[idx, :]
    ...
    fig.savefig("figures/seqdeft_path_posterior.svg", dpi=300)
